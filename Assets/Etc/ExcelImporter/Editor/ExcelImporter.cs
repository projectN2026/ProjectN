...
public class ExcelImporter : AssetPostprocessor
{
	...
	static void ImportExcel(string excelPath, ExcelAssetInfo info)
	{
        ...
        {
            // private인데 [SerializeField] 없으면 스킵
            if (!assetField.IsPublic && !Attribute.IsDefined(assetField, typeof(SerializeField))) continue;

            string sheetName = FieldNameToSheetName(assetField.Name);
    ...
    // 필드명 검사 가공 함수
    static string FieldNameToSheetName(string fieldName)
    {
        // m_objects → objects / _objects → objects
        if (fieldName.StartsWith("m_")) fieldName = fieldName.Substring(2);
        else if (fieldName.StartsWith("_")) fieldName = fieldName.Substring(1);

        // objects → Objects (PascalCase)
